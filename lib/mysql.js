...
MySQL.prototype.buildSelect = function(model, filter, options) {
  ...
  if (filter) {
    ...
    if (filter.where) {
      var aliasCount = {};
      var joins = this.buildJoins(model, filter.where, tableAlias, aliasCount, model);
...
MySQL.prototype.buildWhere = function(model, where, tableAlias, aliases, options) {
  var whereClause = this._buildWhere(model, where, tableAlias, aliases, options, model);
  if (whereClause.sql) {
 ...
 * @private
 */
MySQL.prototype._buildWhere = function (model, where, tableAlias, aliases, options, joinsPath) {
  if (!where) {
    return new ParameterizedSQL('');
  }
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (var i = 0, n = clauses.length; i < n; i++) {
          var stmtForClause = self._buildWhere(model, clauses[i], tableAlias, aliases, options, joinsPath);
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        let childWhere = self._buildWhere(relation.model, where[key],
          aliases[joinsPath + '.' + relation.model], aliases, options, joinsPath + '.' + relation.model);
...
MySQL.prototype.buildJoins = function (model, where, tableAlias, aliasCount, joinsPath) {
  var self = this;
  var props = self.getModelDefinition(model).properties;
  ...
  for (var key in where) {
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (var i = 0, n = clauses.length; i < n; i++) {
          var stmtForClause = self.buildJoins(model, clauses[i], tableAlias, aliasCount, joinsPath);
    ...
    if (p == null) {
      ...
      if (relations && relations[key]) {
        ...
        aliases[joinsPath + '.' + relation.model] = childTableAlias;
        ...
        if (type == 'belongsTo') {
          ...
          aliases[joinsPath + '.' + relation.through] = throughModelAlias;
        ...
        var recursiveResult = self.buildJoins(relation.model, where[key],
          childTableAlias, aliasCount, joinsPath + '.' + relation.model);
      ...
      if (key.indexOf('.') > -1) {
        ...
        keyOrders.slice(0, -1).forEach((keyOrder, idx) => {
          ...
          if (relations && relations[subModel]) {
            ...
            joinsPath += '.' + relation.model;
            alias = aliases[joinsPath];
...
MySQL.prototype.count = function(model, where, options, cb) {
  ...
  var aliasCount = {};
  var joins = this.buildJoins(model, where, tableAlias, aliasCount, model);
  var aliases = {};
  if (joins.joinStmt.sql) {
